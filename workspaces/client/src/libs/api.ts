...
export const getWriteups = async (
  query: InferRequestType<typeof apiClient.api.v1.writeups.$get>['query']
) => {
  const res = await apiClient.api.v1.writeups.$get({query});
  const data = await res.json();
  if (res.ok && data.success) return data.data.map((w) => ({
    ...w,
    createdAt: new Date(w.createdAt),
    updatedAt: new Date(w.updatedAt),
    tags: (w.tags ?? []).map(wt => ({
...
export const createWriteup = async (
  payload: InferRequestType<typeof apiClient.api.v1.writeups.$post>['json']
) => {
...
export const getWriteup = async (
  id: number
) => {
  const res = await apiClient.api.v1.writeups[":id"].$get({ param: { id: id.toString() } });
  const data = await res.json();
  if (res.ok && data.success) return {
    ...data.data,
    createdAt: new Date(data.data.createdAt),
...
export const getWriteupContent = async (
  id: number,
  password?: string
) => {
  const res = await apiClient.api.v1.writeups[":id"].content.$get({ param: { id: id.toString() }, header: {'x-password': password } });
  const data = await res.json();
  if (res.ok && data.success) return data.data;
  if (!data.success && data.message) throw new ApiError(data.message, res.status);
  throw new ApiError("Failed to fetch", res.status);
...
export const deleteWriteup = async (
  id: number
) => {
